...
class StorageNotice extends MySimpleStorage {
		...
		for (var serialized of res.data) {
			...
			for (var param_name in postprocess_filter_params) {
				...
				if (param_con == "beforenow") {
					...
					return {
						action: "error",
						message: "Cannot understand postprocess param" + param_name + param_con
					}
				}
	...
	async hideNoticeByID(notice_id) {
		let res = await super._update_sql({ "id": notice_id, "date_hide": MyUtils.localDateToFloatingTimeStr(new Date(), true) }, "mynotices");
		return res;
	}
