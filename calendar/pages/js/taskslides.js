...
taskSlidesModel.trySaveLog = function (forced) {
    ...
    Promise.all(tmppromises).then((resp_data_arr) => {
        for (var i in resp_data_arr) {
            ...
            var date_str = resp_data_arr[i].title.slice("dailycheck_".length);
...
document.addEventListener("DOMContentLoaded", function () {
    ko.applyBindingsToNode(document.getElementById("currentTaskNameA"), {
        text: taskSlidesModel.selectedTaskName
    });
    ko.applyBindingsToNode(document.getElementById("timerSlides"), {
        text: taskSlidesModel.countDownV
    });
    ko.applyBindingsToNode(document.getElementById("taskButtonNeedFlicker"), {
        css: { animateFlicker: taskSlidesModel.isWarning }
    });
    ko.applyBindingsToNode(document.getElementById("slideReasonTextArea"), {
        textInput: taskSlidesModel.taskLogSwitchReason
    });
    ko.applyBindingsToNode(document.getElementById("selectedDetailsTextArea"), {
        value: taskSlidesModel.selectedTaskDetails
    });
});